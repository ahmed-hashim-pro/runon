...
_runon() {{
    ...
    case "$prev" in
        ...
        --program|-p) COMPREPLY=($(compgen -W "$(runon list programs 2>/dev/null | awk '{{print $1}}')" -- "$cur")); return;;
...
_runon() {{
    ...
    case "${{words[CURRENT-1]}}" in
        --program|-p) compadd -- ${{(f)"$(runon list programs 2>/dev/null | awk '{{print $1}}')"}}; return;;
